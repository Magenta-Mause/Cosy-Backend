...
public class GameServerPermissionsUtility {
        ...
        public static List<GameServerAccessPermission> extractUserPermissions(
                        String userId, List<GameServerAccessGroupEntity> accessGroups) {
                if (accessGroups == null) {
                        return List.of();
                }
                List<GameServerAccessGroupEntity> accessGroupsOfUser = accessGroups.stream()
                                .filter(
                                                gameServerAccessGroup -> gameServerAccessGroup.getUsers().stream()
                                                                .anyMatch(
        ...
        public static boolean isOwnerOrHasPermission(
                ...
                List<GameServerAccessPermission> userPermissions = extractUserPermissions(user.getUuid(),
                                gameServer.getAccessGroups());
                return can(permission, userPermissions);
        }
