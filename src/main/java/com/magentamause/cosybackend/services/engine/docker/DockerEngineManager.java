...
public class DockerEngineManager implements EngineManager, Closeable {
  ...
  private final List<BiConsumer<GameServerStatusUpdateEventType, String>> statusListeners = new CopyOnWriteArrayList<>();
  ...
  private final List<BiConsumer<GameServerStatusUpdateEventType, String>> failListeners = new CopyOnWriteArrayList<>();

  private final Map<String, Supplier<GameServerDto.GameServerStatus>> statusSuppliers = new ConcurrentHashMap<>();
  ...
  private void startEventListener() {
    eventCallback = new ResultCallback.Adapter<>() {
      @Override
  ...
  public void stopAndRemove(GameServerEntity serverConfig) {
    Container container = findContainer(serverConfig)
        .orElseThrow(
  ...
  public void attachLogListener(
    ...
    ResultCallback.Adapter<Frame> callback = new ResultCallback.Adapter<>() {
      ...
      public void onNext(Frame frame) {
        ...
        GameServerLogMessageEntity logMessage = GameServerLogMessageEntity.builder()
            .message(message)
            .level(
  ...
  private HostConfig buildHostConfig(GameServerEntity serverConfig) {
    ...
    if (serverConfig.getVolumeMounts() != null && !serverConfig.getVolumeMounts().isEmpty()) {
      List<Bind> binds = serverConfig.getVolumeMounts().stream()
          .map(
              v -> new Bind(
  ...
  private void ensureImagePresent(
    ...
    // TODO: refactor
    boolean exists = client.listImagesCmd().withImageNameFilter(image).exec().stream()
        .anyMatch(
            img -> {
